...
void simulacao_completa(int log_level, int sim_id, const char* output_suffix, atomic_int* global_log_count, bool dealer_analysis, bool freq_analysis_26, bool freq_analysis_70, bool freq_analysis_A, bool split_analysis, pthread_mutex_t* dealer_mutex, pthread_mutex_t* freq_mutex, pthread_mutex_t* split_mutex) {
        ...
        while (shoe.topo <= limite_penetracao) {
            ...
            for (int pj = 0; pj < total_maos; ++pj) {
                ...
                for (int h = 0; h < hand_count; ++h) {
                    ...
                    Mao *m = &hands[h];
                    Mao *split_result = jogar_mao(m, &shoe, dealer_up_rank, nova, &running_count, &true_count, &shoe_counter);
                    if (split_result) {
